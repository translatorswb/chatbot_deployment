...
def train(request):
    ...
    out = Popen(['/usr/local/bin/rasa', 'train', '--data', f'{current_repo_path}/data', '-c', f'{current_repo_path}/config.yml', '-d', f'{current_repo_path}/domain.yml', '--out', f'{current_repo_path}/models'])
    ...
    out = Popen(['curl', '-k', '-F', f'model=@{current_repo_path}/models/{latest_model}', 'http://rasa-x:5002/api/projects/default/models?api_token={API_TOKEN}'], stdout=PIPE)
    (output, err) = out.communicate()  
...
def generate(request):
    ...
    out_quiz_data_filepath = os.path.join(output_directory_path, 'quiz_data.json')
